...
export default function LeaveCalendarTable({
  ...
  const handleDragEnd = (result) => {
    ...
    // Get the department from droppableId
    const deptId = source.droppableId.replace('dept-', '');
    
    if (source.droppableId !== destination.droppableId) return;

    const deptEmployees = localEmployees.filter(e => e.department_id === deptId);
    const otherEmployees = localEmployees.filter(e => e.department_id !== deptId);
    
    const reordered = Array.from(deptEmployees);
    const [removed] = reordered.splice(source.index, 1);
    reordered.splice(destination.index, 0, removed);

    const newEmployees = [...otherEmployees, ...reordered];
        ...
        <table className="min-w-full">
          ...
          <tbody>
            {departments.map((dept) => {
              const deptEmployees = localEmployees.filter(e => e.department_id === dept.id);
              return (
                              ...
                              <td className="sticky left-[80px] z-10 bg-white px-3 py-1 text-sm text-gray-800 border-r border-b border-gray-200">
                                <div className="flex items-center gap-1">
                                  ...
                                  <Button
                                    variant="outline"
                                    size="icon"
                                    className="h-6 w-6 bg-blue-50 hover:bg-blue-100 border-blue-200 flex-shrink-0"
