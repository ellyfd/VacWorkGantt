      ...
      <table className="min-w-full border-collapse">
          <thead>
            <tr className="bg-gray-50">
              <th className="sticky left-0 z-30 bg-gray-50 px-2 py-2 text-left text-xs font-semibold text-gray-600 border-r border-b border-gray-200 min-w-[100px] shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                姓名
              </th>
            {days.map((d, idx) => (
              <th 
            ...
            return employeesToShow.map((emp) => {
              ...
              return (
                <tr key={emp.id} className="hover:bg-gray-50/50">
                        <td className={`sticky left-0 z-20 px-1 py-1 text-xs text-gray-800 border-r border-b border-gray-200 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] ${isCurrentUser ? 'bg-yellow-100' : 'bg-white'}`}>
                          <div>{emp.name}</div>
