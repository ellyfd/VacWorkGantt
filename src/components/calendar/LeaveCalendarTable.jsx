...
export default function LeaveCalendarTable({
  ...
  return (
    <div className="overflow-x-auto bg-white rounded-xl shadow-sm border border-gray-200 -mx-6 md:mx-0">
      <table className="min-w-full">
          <thead>
            <tr className="bg-gray-50">
              <th className="sticky left-0 z-20 bg-gray-50 px-3 md:px-2 py-2 text-left text-xs font-semibold text-gray-600 border-r border-b border-gray-200 min-w-[80px] md:min-w-[70px]">
                姓名
              </th>
            {days.map((d, idx) => (
              <th 
                key={idx} 
                className={`px-1 md:px-0.5 py-1 text-center text-xs font-semibold border-r border-b border-gray-200 min-w-[32px] md:min-w-[28px] ${
            ...
            return employeesToShow.map((emp) => {
              ...
              return (
                <tr key={emp.id} className="hover:bg-gray-50/50">
                        <td className={`sticky left-0 z-10 px-2 md:px-1 py-1.5 md:py-1 text-xs text-gray-800 border-r border-b border-gray-200 ${isCurrentUser ? 'bg-yellow-100' : 'bg-white'}`}>
                          <div className="text-xs md:text-xs font-medium">{emp.name}</div>
                          <div className="text-[10px] md:text-[10px] text-gray-500">{emp.english_name || ''}</div>
                        </td>
                  {days.map((d, idx) => {
